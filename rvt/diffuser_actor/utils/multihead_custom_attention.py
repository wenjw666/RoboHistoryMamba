...
def multi_head_attention_forward(query,  # type: Tensor
    ...
    if use_separate_proj_weight is not True:
        if qkv_same:
            ...
            if _b is not None:
                _b = _b[_start:_end]
            q = F.linear(query, _w.to(query.device), _b.to(query.device))
            ...
            if _b is not None:
                _b = _b[_start:_end]
            k = F.linear(key, _w.to(key.device), _b.to(key.device))
            ...
            if _b is not None:
                _b = _b[_start:]
            v = F.linear(value, _w.to(value.device), _b.to(value.device))
    else:
    ...
    attn_output = F.linear(attn_output, out_proj_weight.to(attn_output.device), out_proj_bias.to(attn_output.device))
