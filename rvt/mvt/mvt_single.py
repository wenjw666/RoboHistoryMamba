...
class MVT(nn.Module):
    def __init__(
        ...
        if not self.no_feat:
            ...
            if self.cvx_up:  # whether to use learned convex upsampling
                feat_fc_dim += self.input_dim_before_seq
            else:
            ...
            def get_feat_fc(
                ...
                """
                获取线性投影层，_feat_in_size->_feat_out_size
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.add_proprio:  # 加入本体感知数据
            p = self.proprio_preprocess(proprio)  # [B,4] -> [B,64] 对齐维度
        ...
        x = self.fc_bef_attn(ins)  # 调整至atten-dim
        ...
        #  x[ bs * self.num_img, self.input_dim_before_seq, num_pat_img, num_pat_img ]->u ->trans[bs, self.num_img, h, w]
        # 预测得到平移量trans
        if self.cvx_up:  # whether to use learned convex upsampling
            trans = self.up0(x)  # 上采样
        ...
        ## 提取feat对trans外的目标变量进行预测，旋转、开合、碰撞等
        if not self.no_feat:  # whether to return features or not
            if self.feat_ver == 0:  # whether to max pool final features or use soft max values using the heamtmap

                # 由trans生成预测的heatmap trans -> hm [bs * self.num_img, 1, h, w]
                ...
                if self.cvx_up:  # whether to use learned convex upsampling
                    ...
                    #  hm -> _hm[bs * self.num_img, self.img_patch_size * self.img_patch_size, num_pat_img * num_pat_img]
                    assert _hm.shape == (
                        bs * self.num_img,
            ...
            elif self.feat_ver == 1:  # whether to max pool final features or use soft max values using the heamtmap
                # get wpt_local while testing
                if not self.training:  # 推理时从预测的trans中的得到该数据
            ...
            # 由feat预测结果
            if self.rot_ver == 0:  # version of the rotation prediction network
                feat = self.feat_fc(feat)  # 一次全部预测完
                out = {"feat": feat}
            elif self.rot_ver == 1:  # 分步预测
                ...
                if self.training:
                    rot_x = rot_x_y[..., 0].view(bs, 1)  # TODO ??
                else:
                    # sample with argmax
                ...
                out = {
                    "feat_ex_rot": feat_ex_rot,  # grip_q+collision_q
                    "feat_x": feat_x,  # rot_q
                    "feat_y": feat_y,  # rot_q
                    "feat_z": feat_z,  # rot_q
                }
