...
def place_pc_in_cube(
    ...
    scale = 2 / max(x_len, y_len, z_len) # place the point cloud (pc) inside a cube of size (2, 2, 2)
    if app_pc is None:  # 如果app_pc为输入的点则对点的位置进行缩放并返回，否则为整个pc
        app_pc = torch.clone(pc)
