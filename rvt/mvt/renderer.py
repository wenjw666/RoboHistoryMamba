...
class BoxRenderer:
    ...
    def __call__(self, pc, feat, fix_cam=True, dyn_cam_info=None):
        ...
        """
        assert pc.shape[-1] == 3
        assert len(pc.shape) == 2  # torch.Size([54339, 3])
        assert len(feat.shape) == 2  # torch.Size([54339, 6])
    ...
    def get_pt_loc_on_img(self, pt, fix_cam=True, dyn_cam_info=None):
        ...
        if fix_cam:
            ...
            )[..., 0:2]  # torch.Size([5, 4, 2]) 只保留前两个维度的坐标，即 (x, y)，因为屏幕坐标通常是二维的
            if len(fix_cam) == 1:  # 5
                pt_scr = pt_scr.unsqueeze(0)

            pt_scr = torch.transpose(pt_scr, 0, 1)
            # transform from camera screen to image index
            h, w = self.img_size  # (220, 220)
            # (bs * np, num_img, 2)
            fix_pt_img = pt_scr - torch.tensor((1 / w, 1 / h)).to(pt_scr.device)  # 处理为h-1 w-1
            fix_pt_img = fix_pt_img.view(bs, np, len(fix_cam), 2)  #  torch.Size([4, 1，5, 2])
