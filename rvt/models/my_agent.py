...
import rvt.mvt.config_mamba as config_mamba
...
def print_loss_log(agent):
    ...
    temp = {k: v for k, v in out.items() if "image" not in k}
    pprint.pprint(temp)
    return out

def manage_evaluation_log(
        agent,
        eval_log,
        reset_log,
):
    if not hasattr(agent, "eval_log") or reset_log:
        agent.eval_log = {}

    for key, val in eval_log.items():
        if "image" in key:
            agent.eval_log[key] = val
            continue
        if key in agent.eval_log:
            agent.eval_log[key].append(val)
        else:
            agent.eval_log[key] = [val]

def print_evaluation_log(agent):
    out = {}
    for key, val in agent.eval_log.items():
        if "image" in key: 
            out[key] = val
            continue
        out[key] = np.mean(np.array(val))
...
class MyAgent:
    def __init__(
        ...
        """
        #######################################################
        # mamba_cfg = config_mamba.get_cfg_defaults()
        # self.mamba_pipeline = MyMambaPipeline(**mamba_cfg.mymamba).cuda()
    ...
    def update(
        ...
        assert replay_sample["low_dim_state"].shape[1:] == (
            1,
            self._net_mod.proprio_dim,  # 暂时没用
        )
        torch.autograd.set_detect_anomaly(True)  # 追踪异常模块
        ...
        action_grip = action_rot_grip[:, -1]  # (b,) 夹爪状态   # 暂时没用
        ...
        with torch.no_grad():
            ...
            bs = len(pc)
            nc = self._net_mod.num_camera
            h = w = self._net_mod.img_height
        ...
        with autocast(enabled=self.amp):
            ...
            for n in range(num_history_frame):
                history_pc[n] = history_pc[n].view(bs, nc, h, w, 3).permute(0, 4, 1, 2, 3).cuda()
                history_img_feats[n] = history_img_feats[n].view(bs, nc, h, w, 3).permute(0, 4, 1, 2, 3).cuda()
            history_pc = torch.cat(history_pc, dim=2)
            history_img_feats = torch.cat(history_img_feats, dim=2)

            pre_bbox, pre_points = self._network([history_img_feats, history_pc, lang_goal_embs])
            ...
            for n in range(bs):
                ...
                for cname in ["front", "left_shoulder", "right_shoulder", "wrist"]:
                    ...
                    # print(f"pre_points shape:{pre_points[-1].shape}， bs:{bs}")
                    # todo bs会变成从4变成3？？？
                    point, img_for_tb = self._network.get_action_trans_single_pcd(pre_points[-1][n][i], wpt[n].to("cpu"), replay_sample[
        ...
        if backprop:
            with autocast(enabled=self.amp):
                ...
                trans_loss_point_4 = nn.functional.smooth_l1_loss(pre_bbox[-2],
                                                                  torch.stack(wpt, dim=0).to("cuda:0"))
                total_loss = trans_loss_point_1 + trans_loss_point_2 + 100*trans_loss_point_3 + 100*trans_loss_point_4
                print(
                    f"loss:{total_loss},loss1:{trans_loss_point_1},loss2:{trans_loss_point_2},loss3:{trans_loss_point_3},loss4:{trans_loss_point_4}")
            ...
            loss_log = {
                "total_loss": total_loss.item(),
                "trans_loss_point_1": trans_loss_point_1.item(),
                "trans_loss_point_2": trans_loss_point_2.item(),
                "trans_loss_point_3": trans_loss_point_3.item(),
                "trans_loss_point_4": trans_loss_point_4.item(),
                "lr": self._optimizer.param_groups[0]["lr"],
                "image": imgs_for_tb,
        ...
        if eval_log:
            with torch.no_grad():
                ...
                total_loss = trans_loss_point_1 + trans_loss_point_2 + 100*trans_loss_point_3 + 100*trans_loss_point_4
                print(
                    f"[EVAL] loss:{total_loss},loss1:{trans_loss_point_1},loss2:{trans_loss_point_2},loss3:{trans_loss_point_3},loss4:{trans_loss_point_4}")
                ...
                # )
                eval_log = {
                        "env_total_loss": total_loss.item(),
                        "env_trans_loss_point_1": trans_loss_point_1.item(),
                        "env_trans_loss_point_2": trans_loss_point_2.item(),
                        "env_trans_loss_point_3": trans_loss_point_3.item(),
                        "env_trans_loss_point_4": trans_loss_point_4.item(),
                        "env_image": imgs_for_tb
                    }
                manage_evaluation_log(self, eval_log, reset_log=reset_log)

                return_out.update(eval_log)
